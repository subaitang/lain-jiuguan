      ...
      if (mapData) {
        ...
        if (mapData.nodes) {
          mapData.nodes.forEach(node => {
            const posX = startX + node.x * cellSize;
            const posY = startY + node.y * cellSize;
            if (posX < -cellSize || posY < -cellSize || posX > canvas.width || posY > canvas.height) return;

            ctx.fillStyle = 'rgba(255, 255, 0, 0.2)';
            ctx.beginPath();
            ctx.arc(posX + cellSize / 2, posY + cellSize / 2, cellSize / 3, 0, Math.PI * 2);
            ctx.fill();
            ...
            ctx.fillText(node.icon || '?', posX + cellSize / 2, posY + cellSize / 2);
          });
        }
