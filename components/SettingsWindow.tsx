      ...
      <NaviWindow title={t('settings_title', language)} className="w-full h-full md:w-[950px] md:h-[750px]" onClose={onClose}>
        <div className="flex flex-col h-full font-['Share_Tech_Mono'] text-[color:var(--lain-cyan)] bg-black/90">
          <div className="flex border-b border-[color:var(--lain-cyan)]/30 overflow-x-auto no-scrollbar bg-black/50 shrink-0">
            ...
            <TabButton id="variables" label="VARS" icon={Link} />
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 scrollbar-thin">
            ...
            {activeTab === 'variables' && (
              <div className="space-y-4 animate-in fade-in duration-300 max-w-2xl mx-auto">
                <h3 className="font-bold border-b border-[color:var(--lain-cyan)]/30 pb-2 mb-4 flex items-center gap-2 uppercase tracking-[0.2em]"><Link size={18} /> GLOBAL_VARIABLES</h3>
                <div className="space-y-2">
                  {Object.entries(settings.variables || {}).map(([key, val]) => (
                    <div key={key} className="flex gap-2">
                      <input value={key} readOnly className="w-1/3 bg-black/50 border border-[color:var(--lain-cyan)]/30 p-2 text-xs font-mono opacity-70" />
                      <input value={val} onChange={(e) => onUpdateSettings({ ...settings, variables: { ...settings.variables, [key]: e.target.value } })} type="password" placeholder="Value..." className="flex-1 bg-black border border-[color:var(--lain-cyan)]/50 p-2 text-xs focus:border-[color:var(--lain-cyan)]" />
                      <button onClick={() => {
                        const newVars = { ...settings.variables };
                        delete newVars[key];
                        onUpdateSettings({ ...settings, variables: newVars });
                      }} className="p-2 text-red-500 hover:bg-red-900/20 border border-transparent hover:border-red-500"><Trash2 size={14} /></button>
                    </div>
                  ))}
                  <div className="flex gap-2 border-t border-[color:var(--lain-cyan)]/20 pt-4">
                    <input id="newVarKey" placeholder="NEW_VAR_NAME" className="w-1/3 bg-black border border-[color:var(--lain-cyan)]/50 p-2 text-xs uppercase" />
                    <button onClick={() => {
                      const keyInput = document.getElementById('newVarKey') as HTMLInputElement;
                      const key = keyInput.value.trim().toUpperCase();
                      if (key && !settings.variables?.[key]) {
                        onUpdateSettings({ ...settings, variables: { ...(settings.variables || {}), [key]: '' } });
                        keyInput.value = '';
                      }
                    }} className="flex-1 border border-dashed border-[color:var(--lain-cyan)] hover:bg-[color:var(--lain-cyan)] hover:text-black text-xs font-bold uppercase tracking-widest">+ ADD VARIABLE</button>
                  </div>
                </div>
                <div className="p-4 border border-[color:var(--lain-cyan)]/30 bg-[color:var(--lain-cyan)]/5 text-[10px] opacity-70">
                  <Info size={12} className="inline mr-2" />
                  Variables can be used in API Key fields as <code>{'${VAR_NAME}'}</code>. They will be resolved at runtime.
                </div>
              </div>
            )}
