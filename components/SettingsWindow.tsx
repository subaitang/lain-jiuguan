...
import { AppSettings, LoreEntry, ChatSession, PersonaSettings, GroupSettings, CustomExtension, VisualAsset } from '../types';
...
import { Trash2, Save, FileText, Wifi, RefreshCw, CheckCircle, AlertCircle, Server, Plus, User, AlertTriangle, Download, Upload, Cpu, Database, Globe, Mic, Image as ImageIcon, Users, Puzzle, Languages, Palette, MessageSquare, X, Volume2, Speech, Play, Search, MapPin, Video, Zap, BrainCircuit, Type, FileImage, Radio, Edit2, ChevronDown, ChevronUp, FileJson, Router, Flag, Hexagon, Layers, Shuffle, Sparkles, Heart, Frown, Network, MousePointer2, TextCursor, Loader, Award, Link, Film, HeartOff, Ghost, Info, Smile } from 'lucide-react';
...
export const SettingsWindow: React.FC<SettingsWindowProps> = ({ 
    ...
    const [apiLayer, setApiLayer] = useState<'generation' | 'translation' | 'memory' | 'persona'>('generation');
    ...
    const handleImportPersona = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const reader = new FileReader(); reader.onload = (ev) => {
            try { const json = JSON.parse(ev.target?.result as string); const d = json.data || json; 
            ...
            onUpdateSettings({ ...settings, characterLibrary: [...settings.characterLibrary, newP], activeTargetId: newP.id }); setEditingPersonaId(newP.id); audio.playConfirmSound(); alert("Imported."); } catch(e) { alert("Fail."); }
    ...
    const getCurrentApiConfig = () => { if(apiLayer==='generation') return settings.api; if(apiLayer==='translation') return settings.translation||{source:'google', apiKey:'', baseUrl:'', modelName:''}; if(apiLayer==='memory') return settings.memoryApi||settings.api; return settings.personaConfig||settings.api; };
