...
import {
  ...
  generateUserStatsFromInput,
} from "../services/geminiService";
import { AppSettings, Message, RPStats } from "../types";
...
export const RPStatusViewer: React.FC<RPStatusViewerProps> = ({
    ...
    if (activeTab === "user") {
      ...
      if (selectedNpcId === "active") {
        const updatedRegistry = {
          ...npcRegistry,
          [settings.activeTargetId]: tempStats,
        };
  ...
  const handleGeneratePlayerRandom = async () => {
    ...
    try {
      const worldInfo =
        ...
        (settings.ui.wallpaper ? " with visual context" : "");
      const stats = await generateRandomUserStats(
        config,
  ...
  const handleGeneratePlayerKeyword = async () => {
    ...
    try {
      const stats = await generateUserStatsFromInput(
        keyword,
        config,
        abortControllerRef.current.signal,
        targetLangName,
  ...
  const handleGenerateTarget = async () => {
    ...
    const activePersona = settings.characterLibrary.find(
      (p) => p.id === settings.activeTargetId,
    );
    ...
    if (selectedNpcId === "active" && activePersona) {
      ...
      promptDescription =
        "Generate stats for a random NPC in this world context.";
    }
    ...
    try {
      ...
      if (stats && !abortControllerRef.current.signal.aborted) {
        const targetId =
          selectedNpcId === "active" ? settings.activeTargetId : selectedNpcId;
    ...
    >
      <div className="flex flex-col h-full bg-black/90 text-[color:var(--lain-cyan)] font-['Share_Tech_Mono']">
        ...
        <div className="flex-1 overflow-y-auto p-4 scrollbar-thin">
          ...
          {activeTab === "npc" && (
            <div className="mb-4">
              <label className="text-[9px] font-bold tracking-widest opacity-60 mb-1 block uppercase">
                {t("rp_active_target", lang)}
              </label>
              <div className="relative group">
                  ...
                  {Object.entries(npcRegistry).map(([id, stats]) => {
                    ...
                    return (
                      <option key={id} value={id}>
                        [CACHED]: {npcStats.name || "Unknown NPC"}
                      </option>
                    );
                  })}
                </select>
                <ChevronDown
                  size={14}
                  className="absolute right-2 top-2.5 pointer-events-none opacity-70"
                />
              </div>
            </div>
          )}
            ...
            {isGenerating && (
              <div className="flex items-center gap-2 text-xs animate-pulse text-yellow-400 justify-center py-2 bg-yellow-900/20 mb-2 border border-yellow-500/30">
                ...
                <span className="tracking-widest">
                  NEURAL GENERATION IN PROGRESS...
                </span>
                <button
                  onClick={cancelGeneration}
                  className="ml-2 border border-red-500 text-red-500 px-2 hover:bg-red-500 hover:text-white text-[9px] font-bold"
                >
                  ABORT
                </button>
              </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={(e) =>
                      e.key === "Enter" && handleGeneratePlayerKeyword()
                    }
                  />
                  <button
          ...
          <div className="flex gap-4 mb-4">
            <div className="w-20 h-20 border-2 border-[color:var(--lain-cyan)] flex flex-col items-center justify-center bg-[color:var(--lain-cyan)]/5 shrink-0 shadow-[0_0_15px_var(--lain-cyan)] relative overflow-hidden group">
              ...
              <span className="text-[10px] opacity-60 font-bold tracking-widest">
                {t("rp_lvl", lang)}
              </span>
            ...
            <div className="flex-1 space-y-1.5 py-1">
              <StatBar
                label={t("rp_hp", lang)}
                current={currentStats?.hp.current || 0}
                max={currentStats?.hp.max || 10}
                color="bg-red-600 shadow-[0_0_8px_rgba(220,38,38,0.6)]"
              />
              <StatBar
                label={t("rp_mp", lang)}
                current={currentStats?.mp.current || 0}
                max={currentStats?.mp.max || 10}
                color="bg-blue-600 shadow-[0_0_8px_rgba(37,99,235,0.6)]"
              />
              <StatBar
                label={t("rp_xp", lang)}
                current={currentStats?.xp || 0}
                max={currentStats?.maxXp || 100}
              ...
              {activeTab === "npc" && (
                <div className="flex items-center gap-2 text-[10px] w-full mt-2 animate-in slide-in-from-left-2">
                  <span className="w-8 font-bold flex items-center gap-1 text-pink-400">
                  ...
                  <div className="flex-1 h-2 bg-gray-900 border border-pink-900 relative overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, currentStats?.affection || 0)}%`,
                      }}
                    />
                  ...
                  <span className="w-12 text-right font-mono text-pink-300">
                    {currentStats?.affection || 0}%
                  </span>
                </div>
              )}
              ...
              {activeTab === "user" && (
                <div className="flex items-center gap-2 text-[10px] w-full mt-2">
                  <span className="w-8 font-bold flex items-center gap-1 text-orange-400">
                  ...
                  <div className="flex-1 h-2 bg-gray-900 border border-orange-900 relative overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, currentStats?.limitGauge || 0)}%`,
                      }}
                    />
                  </div>
                  <span className="w-12 text-right font-mono text-orange-300">
                    {currentStats?.limitGauge || 0}%
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 gap-2 mb-4 text-xs">
            {["race", "gender", "class", "alignment"].map((field) => (
              <div
                key={field}
                className="border border-[color:var(--lain-cyan)]/30 p-2 bg-[color:var(--lain-cyan)]/5 flex items-center gap-2 relative group"
              >
                ...
                <span className="font-bold text-[9px] opacity-50 uppercase tracking-wider">
                  {field}:
                </span>
                {isEditing ? (
                  <input
                  ...
                  <span className="truncate">
                    {(currentStats as any)?.[field] || "Unknown"}
                  </span>
                )}
              </div>
            ))}
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {Object.entries(currentStats.attributes).map(([key, val]) => (
                  <div
                    key={key}
                    className="bg-black border border-[color:var(--lain-cyan)]/30 p-2 text-center group hover:bg-[color:var(--lain-cyan)]/10 transition-colors relative"
                  >
                    <div className="text-[9px] opacity-60 font-bold tracking-wider">
                      {key}
                    </div>
                    {isEditing ? (
                      <input
