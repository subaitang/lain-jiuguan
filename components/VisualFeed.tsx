...
import { PersonaSettings, Message, AppSettings, VisualAsset } from '../types';
import { Upload, RefreshCw, Image as ImageIcon, Search, Check, Sliders, Edit3, Video, Loader2, X, Activity, Wifi, Shield, Radio, Mic } from 'lucide-react';
import { audio } from '../services/audioEngine';
import { generateVisualDescription, editImage, generateImage, generateVeoVideo } from '../services/geminiService';
 ...
 */
const compressAndResizeImage = (file: File, maxWidth = 1024): Promise<string> => {
...
const VideoLinkHUD: React.FC<{ name: string, ip?: string, isLoading: boolean }> = ({ name, ip, isLoading }) => {
            ...
            {isLoading && (
                <div className="self-center flex flex-col items-center gap-2">
                    <div className="px-4 py-1 border border-white/50 bg-black/80 animate-pulse">
                        <span className="text-xs font-bold tracking-[0.5em] text-white">RE-BUFFERING DATA...</span>
                    </div>
...
export const VisualFeed: React.FC<VisualFeedProps> = ({ activePersona, messages, apiKey, onUpdatePersona, opacity, setOpacity, settings, onUpdateSettings }) => {
    const [visibleSrc, setVisibleSrc] = useState<string>(activePersona.visualImage || "https://picsum.photos/seed/lain/800/800");
    ...
    const handleGenRequest = async (e: React.FormEvent) => {
        ...
        try {
            const url = await generateImage(searchQuery, apiKey, genMode === 'pro' ? 'pro' : 'nano');
            if (url) {
                setVisibleSrc(url); 
            ...
            {isSearching ? (
                <div className="w-full h-full bg-black/90 p-4 flex flex-col z-30 animate-in fade-in">
                    <div className="flex justify-between items-center mb-4 border-b border-[color:var(--lain-cyan)]/30 pb-2">
                        <h3 className="text-xs font-bold tracking-widest text-[color:var(--lain-cyan)]">{t('vf_title', lang)}</h3>
                        <button onClick={() => setIsSearching(false)} className="text-red-500"><X size={14}/></button>
                    </div>
                    <form onSubmit={handleGenRequest} className="space-y-4">
                        <div className="flex gap-2">
                            {['nano', 'pro', 'veo'].map(m => (
                                <button key={m} type="button" onClick={() => setGenMode(m as any)} className={`flex-1 py-1 text-[10px] border ${genMode === m ? 'bg-[color:var(--lain-cyan)] text-black' : 'border-[color:var(--lain-cyan)]/30'}`}>{m.toUpperCase()}</button>
                            ))}
                        </div>
                        <textarea value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="w-full h-24 bg-black border border-[color:var(--lain-cyan)]/30 p-2 text-xs outline-none focus:border-[color:var(--lain-cyan)] resize-none" placeholder={t('vf_prompt_gen', lang)} />
                        <button type="submit" className="w-full py-2 bg-[color:var(--lain-cyan)] text-black font-bold text-xs hover:bg-white transition-colors">GENERATE</button>
                    </form>
                </div>
            ) : (
                <div className="w-full h-full relative">
                    <VideoLinkHUD name={activePersona.name} ip={activePersona.ipAddress} isLoading={isLoading} />
                    
                    {isVideoUrl(visibleSrc) ? (
                        <video src={visibleSrc} className="w-full h-full object-cover" style={{ opacity }} autoPlay loop muted playsInline key={visibleSrc} />
                    ) : (
                        <img src={visibleSrc} className="w-full h-full object-cover" style={{ opacity }} alt="visual" key={visibleSrc} />
                    )}
                    ...
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col gap-2 z-20 pointer-events-auto">
                        <button onClick={() => setIsSearching(true)} className="p-2 bg-black/80 border border-[color:var(--lain-cyan)]/50 hover:bg-[color:var(--lain-cyan)] hover:text-black transition-colors" title="New Generation"><RefreshCw size={14}/></button>
                        <button onClick={() => fileInputRef.current?.click()} className="p-2 bg-black/80 border border-[color:var(--lain-cyan)]/50 hover:bg-[color:var(--lain-cyan)] hover:text-black transition-colors" title="Upload Local"><Upload size={14}/></button>
